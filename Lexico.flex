...
{CONST_FLOAT} {
        ...
        if (valor < -3.4028235E38|| valor > 3.4028235E38) {
            throw new Error("Caracter inválido: <" + yytext() + "> en la línea " + yyline + " excede el límite de tamaño");
        } else {
...
{CONST_STR} {
        String value = yytext();
        if (value.length() > 32) {
            throw new Error("Caracter inválido: <" + yytext() + "> en la linea " + yyline + " excede el límite de caracteres");
        } else {
...
[^] {     throw new Error("Caracter inválido: <" + yytext() + "> en la línea " + yyline);
      }